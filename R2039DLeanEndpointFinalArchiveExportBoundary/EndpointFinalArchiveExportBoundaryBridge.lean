...
/-- Bridge from R2039D final archive/export boundary to dependency audit. -/
def endpoint_final_archive_export_boundary_to_dependency_audit
    ...
    EndpointFinalArchiveExportDependencyAudit R :=
  { auditItemCount := R.finalArchiveExportBoundaryStageCount
    attachedToFinalArchiveExportBoundary := R.recordsOnlyFinalArchiveExportBoundary
    recordsNoRHProof := True
    recordsNoFullRHFormalization := True
    actualLiTarget := R.actualLiTarget }

/-- The bridge preserves the analytic target. -/
theorem endpoint_final_archive_export_boundary_bridge_to_actual_li_target
  ...
  (endpoint_final_archive_export_boundary_to_dependency_audit R).actualLiTarget
