...
/-- Stages in the final submission boundary layer. -/
inductive EndpointFinalSubmissionBoundaryStage where
  | inheritFinalArchiveExportCertificateSlots
  | bindFinalSubmissionHeader
  | bindSubmissionBoundary
  | bindRepositoryEvidenceBoundary
  | bindNoNewMathematicalClaimBoundary
  | recordNoFullRHFormalization
  deriving DecidableEq, Repr

/-- Kinds of final submission boundary records. -/
inductive EndpointFinalSubmissionBoundaryKind where
  | finalArchiveExportCertificateInput
  | finalSubmissionHeaderCarrier
  | submissionBoundaryCarrier
  | repositoryEvidenceBoundaryCarrier
  | noNewMathematicalClaimBoundaryCarrier
  | nonClaimFinalSubmissionCarrier
  deriving DecidableEq, Repr

/-- One R2041D final submission boundary item. It records submission packaging, not new mathematics. -/
structure EndpointFinalSubmissionBoundaryItem (A : AnalyticRealizationObligations) where
  kind : EndpointFinalSubmissionBoundaryKind
  itemIndex : Nat
  finalArchiveExportCertificateCarrier : Type
  finalSubmissionHeaderCarrier : Type
  submissionBoundaryCarrier : Type
  repositoryEvidenceBoundaryCarrier : Type
  noNewMathematicalClaimBoundaryCarrier : Type
  attachedToFinalArchiveExportCertificateSlots : Prop
  recordsFinalSubmissionBoundaryOnly : Prop
...
/-- Main R2041D final submission boundary packet. -/
structure MainAnchoredEndpointFinalSubmissionBoundary
  ...
  finalSubmissionBoundaryStageCount : Nat
  finalSubmissionBoundaryItem : EndpointFinalSubmissionBoundaryItem A
  inheritedFinalArchiveExportCertificateSlots : Type
  finalSubmissionBoundaryCarrier : Type
  recordsOnlyFinalSubmissionBoundary : Prop
  provesEndpointEstimateHere : Bool
  provesExplicitFormulaHere : Bool
  provesFinalTheoremHere : Bool
  provesRHHere : Bool
...
/-- The R2041D final submission boundary layer retains the analytic target. -/
theorem endpoint_final_submission_boundary_to_actual_li_target
    ...
    (V : MainAnchoredEndpointFinalSubmissionBoundary U) : A.actualLiTarget :=
  V.actualLiTarget

/-- R2041D records final submission boundary, not a new proof. -/
theorem r2041_eightieth_step_is_final_submission_boundary_not_final_proof : True :=
  trivial

/-- R2041D does not claim full RH formalization. -/
theorem r2041_eightieth_step_does_not_claim_rh_formalization : True :=
  trivial
