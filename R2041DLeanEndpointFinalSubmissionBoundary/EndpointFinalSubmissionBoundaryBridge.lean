...
/-- Bridge from R2041D final submission boundary to dependency audit. -/
def endpoint_final_submission_boundary_to_dependency_audit
    ...
    EndpointFinalSubmissionDependencyAudit V :=
  { auditItemCount := V.finalSubmissionBoundaryStageCount
    attachedToFinalSubmissionBoundary := V.recordsOnlyFinalSubmissionBoundary
    recordsNoRHProof := True
    recordsNoFullRHFormalization := True
    actualLiTarget := V.actualLiTarget }

/-- The bridge preserves the analytic target. -/
theorem endpoint_final_submission_boundary_bridge_to_actual_li_target
  ...
  (endpoint_final_submission_boundary_to_dependency_audit V).actualLiTarget
