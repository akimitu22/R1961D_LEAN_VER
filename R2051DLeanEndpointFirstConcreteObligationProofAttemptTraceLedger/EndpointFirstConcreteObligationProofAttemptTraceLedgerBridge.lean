...
/-- Bridge from R2051D proof-attempt trace ledger to its dependency audit. -/
def endpoint_first_concrete_obligation_proof_attempt_trace_ledger_to_dependency_audit
    ...
    EndpointFirstConcreteObligationProofAttemptTraceLedgerDependencyAudit AF :=
  { auditItemCount := AF.traceLedgerStageCount
    attachedToExecutableAttemptEnvelope := AF.allTraceEntriesRemainOpen
    recordsEndpointEstimateTrace := AF.recordsEndpointEstimateTrace
    recordsExplicitFormulaTrace := AF.recordsExplicitFormulaTrace
    recordsTransferAssemblyTrace := AF.recordsTransferAssemblyTrace
    recordsFiniteAbelReductionTrace := AF.recordsFiniteAbelReductionTrace
    recordsProofKernelConnectionTrace := AF.recordsProofKernelConnectionTrace
    recordsAllTraceEntriesRemainOpen := AF.allTraceEntriesRemainOpen
    recordsNoClosedProofTrace := AF.suppliesNoClosedProofTrace
    recordsFutureConcreteDischargePermission := AF.permitsFutureConcreteDischargeLayer
    actualLiTarget := AF.actualLiTarget }

/-- The R2051D bridge preserves the analytic target. -/
theorem endpoint_first_concrete_obligation_proof_attempt_trace_ledger_bridge_to_actual_li_target
  ...
  (endpoint_first_concrete_obligation_proof_attempt_trace_ledger_to_dependency_audit AF).actualLiTarget
