...
/-- Bridge from R2038D final package closure certificate slots to dependency audit. -/
def endpoint_final_package_closure_certificate_slots_to_dependency_audit
    ...
    EndpointFinalPackageClosureCertificateDependencyAudit Q :=
  { auditItemCount := Q.finalPackageClosureCertificateStageCount
    attachedToFinalPackageClosureCertificateSlots := Q.recordsOnlyFinalPackageClosureCertificateSlots
    recordsNoRHProof := True
    recordsNoFullRHFormalization := True
    actualLiTarget := Q.actualLiTarget }

/-- The bridge preserves the analytic target. -/
theorem endpoint_final_package_closure_certificate_bridge_to_actual_li_target
  ...
  (endpoint_final_package_closure_certificate_slots_to_dependency_audit Q).actualLiTarget
