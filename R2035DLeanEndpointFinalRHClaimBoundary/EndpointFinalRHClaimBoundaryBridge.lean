...
/-- Bridge from R2035D final RH-claim boundary to dependency audit. -/
def endpoint_final_rh_claim_boundary_to_dependency_audit
    ...
    EndpointFinalRHClaimDependencyAudit N :=
  { auditItemCount := N.finalRHClaimBoundaryStageCount
    attachedToFinalRHClaimBoundary := N.recordsOnlyFinalRHClaimBoundary
    recordsNoRHProof := True
    recordsNoFullRHFormalization := True
    actualLiTarget := N.actualLiTarget }

/-- The bridge preserves the analytic target. -/
theorem endpoint_final_rh_claim_boundary_bridge_to_actual_li_target
  ...
  (endpoint_final_rh_claim_boundary_to_dependency_audit N).actualLiTarget
