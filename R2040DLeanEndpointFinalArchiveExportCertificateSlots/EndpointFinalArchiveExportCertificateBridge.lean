...
/-- Bridge from R2040D final archive/export certificate slots to dependency audit. -/
def endpoint_final_archive_export_certificate_slots_to_dependency_audit
    ...
    EndpointFinalArchiveExportCertificateDependencyAudit U :=
  { auditItemCount := U.finalArchiveExportCertificateStageCount
    attachedToFinalArchiveExportCertificateSlots := U.recordsOnlyFinalArchiveExportCertificateSlots
    recordsNoRHProof := True
    recordsNoFullRHFormalization := True
    actualLiTarget := U.actualLiTarget }

/-- The bridge preserves the analytic target. -/
theorem endpoint_final_archive_export_certificate_bridge_to_actual_li_target
  ...
  (endpoint_final_archive_export_certificate_slots_to_dependency_audit U).actualLiTarget
