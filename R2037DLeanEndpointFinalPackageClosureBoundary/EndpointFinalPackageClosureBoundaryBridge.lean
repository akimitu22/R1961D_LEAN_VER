...
/-- Bridge from R2037D final package closure boundary to dependency audit. -/
def endpoint_final_package_closure_boundary_to_dependency_audit
    ...
    EndpointFinalPackageClosureDependencyAudit P :=
  { auditItemCount := P.finalPackageClosureBoundaryStageCount
    attachedToFinalPackageClosureBoundary := P.recordsOnlyFinalPackageClosureBoundary
    recordsNoRHProof := True
    recordsNoFullRHFormalization := True
    actualLiTarget := P.actualLiTarget }

/-- The bridge preserves the analytic target. -/
theorem endpoint_final_package_closure_boundary_bridge_to_actual_li_target
  ...
  (endpoint_final_package_closure_boundary_to_dependency_audit P).actualLiTarget
