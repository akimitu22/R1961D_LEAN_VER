...
/-- Bridge from R2036D final RH-claim certificate slots to dependency audit. -/
def endpoint_final_rh_claim_certificate_slots_to_dependency_audit
    ...
    EndpointFinalRHClaimCertificateDependencyAudit O :=
  { auditItemCount := O.finalRHClaimCertificateStageCount
    attachedToFinalRHClaimCertificateSlots := O.recordsOnlyFinalRHClaimCertificateSlots
    recordsNoRHProof := True
    recordsNoFullRHFormalization := True
    actualLiTarget := O.actualLiTarget }

/-- The bridge preserves the analytic target. -/
theorem endpoint_final_rh_claim_certificate_bridge_to_actual_li_target
  ...
  (endpoint_final_rh_claim_certificate_slots_to_dependency_audit O).actualLiTarget
