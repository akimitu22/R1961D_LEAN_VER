...
/-- Bridge from R2050D executable proof-attempt envelope to its dependency audit. -/
def endpoint_first_concrete_obligation_executable_proof_attempt_envelope_to_dependency_audit
    ...
    EndpointFirstConcreteObligationExecutableProofAttemptEnvelopeDependencyAudit AE :=
  { auditItemCount := AE.executableEnvelopeStageCount
    attachedToProofReadinessChecklist := AE.allAttemptSlotsRemainOpen
    recordsEndpointEstimateAttemptSlot := AE.recordsEndpointEstimateAttemptSlot
    recordsExplicitFormulaAttemptSlot := AE.recordsExplicitFormulaAttemptSlot
    recordsTransferAssemblyAttemptSlot := AE.recordsTransferAssemblyAttemptSlot
    recordsFiniteAbelReductionAttemptSlot := AE.recordsFiniteAbelReductionAttemptSlot
    recordsProofKernelConnectionAttemptSlot := AE.recordsProofKernelConnectionAttemptSlot
    recordsAllAttemptSlotsRemainOpen := AE.allAttemptSlotsRemainOpen
    recordsNoConcreteProofTermsSupplied := AE.suppliesNoConcreteProofTerms
    recordsAttemptTraceLedgerPermission := AE.permitsAttemptTraceLedger
    actualLiTarget := AE.actualLiTarget }

/-- The R2050D bridge preserves the analytic target. -/
theorem endpoint_first_concrete_obligation_executable_proof_attempt_envelope_bridge_to_actual_li_target
  ...
  (endpoint_first_concrete_obligation_executable_proof_attempt_envelope_to_dependency_audit AE).actualLiTarget
