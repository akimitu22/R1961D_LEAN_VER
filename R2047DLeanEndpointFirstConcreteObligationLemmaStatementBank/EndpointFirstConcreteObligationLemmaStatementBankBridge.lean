...
/-- Bridge from R2047D lemma-statement bank to dependency audit. -/
def endpoint_first_concrete_obligation_lemma_statement_bank_to_dependency_audit
    ...
    EndpointFirstConcreteObligationLemmaStatementBankDependencyAudit Z AB :=
  { auditItemCount := AB.lemmaStatementBankStageCount
    attachedToDecompositionSlots := AB.allLemmaStatementsStillOpen
    recordsEndpointEstimateConcreteLemma := AB.recordsEndpointEstimateConcreteLemma
    recordsExplicitFormulaBridgeLemma := AB.recordsExplicitFormulaBridgeLemma
    recordsTransferAssemblyLemma := AB.recordsTransferAssemblyLemma
    recordsFiniteAbelReductionLemma := AB.recordsFiniteAbelReductionLemma
    recordsProofKernelConnectionLemma := AB.recordsProofKernelConnectionLemma
    recordsAllStatementsStillOpen := AB.allLemmaStatementsStillOpen
    actualLiTarget := AB.actualLiTarget }

/-- The R2047D bridge preserves the analytic target. -/
theorem endpoint_first_concrete_obligation_lemma_statement_bank_bridge_to_actual_li_target
  ...
  (endpoint_first_concrete_obligation_lemma_statement_bank_to_dependency_audit AB).actualLiTarget
