...
/-- Bridge from R2045D first concrete proof boundary to dependency audit. -/
def endpoint_first_concrete_proof_obligation_boundary_to_dependency_audit
    ...
    EndpointFirstConcreteProofObligationDependencyAudit Z :=
  { auditItemCount := Z.firstConcreteProofBoundaryStageCount
    attachedToFirstConcreteProofBoundary := Z.recordsFirstConcreteProofBoundary
    recordsEndpointEstimateSubgoal := Z.selectedEndpointEstimateObligation
    recordsExplicitFormulaDependency := Z.selectedExplicitFormulaObligation
    recordsNoEndpointEstimateProofYet := True
    recordsNoRHProofYet := True
    actualLiTarget := Z.actualLiTarget }

/-- The bridge preserves the analytic target. -/
theorem endpoint_first_concrete_proof_obligation_boundary_bridge_to_actual_li_target
  ...
  (endpoint_first_concrete_proof_obligation_boundary_to_dependency_audit Z).actualLiTarget
