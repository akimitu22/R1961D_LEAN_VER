...
/-- Bridge from R2052D trace-replay checkpoint to its dependency audit. -/
def endpoint_first_concrete_obligation_trace_replay_checkpoint_to_dependency_audit
    ...
    EndpointFirstConcreteObligationTraceReplayCheckpointDependencyAudit AG :=
  { auditItemCount := AG.replayCheckpointStageCount
    attachedToProofAttemptTraceLedger := AG.allReplayCheckpointsRemainOpen
    recordsEndpointEstimateReplay := AG.recordsEndpointEstimateReplay
    recordsExplicitFormulaReplay := AG.recordsExplicitFormulaReplay
    recordsTransferAssemblyReplay := AG.recordsTransferAssemblyReplay
    recordsFiniteAbelReductionReplay := AG.recordsFiniteAbelReductionReplay
    recordsProofKernelConnectionReplay := AG.recordsProofKernelConnectionReplay
    recordsAllReplayCheckpointsRemainOpen := AG.allReplayCheckpointsRemainOpen
    recordsNoReplayProofTerm := AG.suppliesNoReplayProofTerm
    recordsLocalCandidateRegistryPermission := AG.permitsLocalCandidateRegistry
    actualLiTarget := AG.actualLiTarget }

/-- The R2052D bridge preserves the analytic target. -/
theorem endpoint_first_concrete_obligation_trace_replay_checkpoint_bridge_to_actual_li_target
  ...
  (endpoint_first_concrete_obligation_trace_replay_checkpoint_to_dependency_audit AG).actualLiTarget
